...
@pytest.fixture
def dependency_manager() -> Generator[DependencyManager, None, None]:
    with (
        patch("src.services.project_setup.dependency_manager.ProcessRunner"),
...
@pytest.mark.asyncio
async def test_sync_dependencies_success(dependency_manager: DependencyManager) -> None:
    # Mock runner.run_command to return success
    dependency_manager.runner.run_command = AsyncMock(  # type: ignore[method-assign]
        side_effect=[
...
@pytest.mark.asyncio
async def test_sync_dependencies_exception_path(dependency_manager: DependencyManager) -> None:
    # Mock runner.run_command to raise an exception during uv sync
    dependency_manager.runner.run_command = AsyncMock(side_effect=Exception("Sync failed"))  # type: ignore[method-assign]

    with patch("src.services.project_setup.dependency_manager.logger") as mock_logger:
        await dependency_manager.sync_dependencies()

        mock_logger.warning.assert_called_with(
            "[ProjectManager] Dependency sync failed: Sync failed"
        )


@pytest.mark.asyncio
async def test_sync_dependencies_install_missing_linters(
    ...
    dependency_manager.runner.run_command.assert_any_call(
        ["uv", "add", "--dev", "ruff", "mypy"], check=True
    )


@pytest.mark.asyncio
async def test_sync_dependencies_uv_add_fails(dependency_manager: DependencyManager) -> None:
    ...
    with patch("src.services.project_setup.dependency_manager.logger") as mock_logger:
        ...
        mock_logger.warning.assert_called_with(
            "[ProjectManager] Dependency sync failed: uv add failed"
        )
